...
DecPack * DecPack_create(int sz)
{
  ...
  // allocate memory for data, should be only sz/2 because each byte
...
void DecPack_insert(DecPack * dp, int val)
{
  ...
  // If the allocated memory is full, double the size, allocate memory
  // for the new size, copy the data, and insert the new value
  int used = dp -> used;
  if (used == dp -> size) 
    {
  ...
  // If used is an even number, the inserted value should use the
  // upper (left) 4 bits.
  // If used is an odd number, the inserted value should use the lower
  // (right) 4 bits.
  // 
  // Make sure you do not lose the data already stored in DecPack
  if ((used % 2) == 0)
    {
      // shifting left adding four zeros for the lower 4 bits
...
int DecPack_delete(DecPack * dp)
{
  ...
  // If used is even, the returned value is the upper (left) 4 bits
  // make sure the returned value is between 0 and 9
  // If used is odd, the returned value is the lower (right) 4 bits
  // make sure the returned value is between 0 and 9
  int val;
