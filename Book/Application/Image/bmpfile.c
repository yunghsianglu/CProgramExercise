...
{
  FILE * fptr    = NULL; // must initialize for cleanUp to work
  BMP_Image *img = NULL;
  fptr = fopen(filename, "r"); // "rb" is unnecessary in Linux
  if (fptr == NULL)
    {
  ...
  if (fread(img -> data, sizeof(char), img -> data_size, fptr) !=
      (img -> data_size))
    {
      // fread fails
      return cleanUp(fptr, img);
  ...
  if (fread(& onebyte, sizeof(char), 1, fptr) != 0)
    {
      // not at the of the file yet and the file still has data
      return cleanUp(fptr, img);
    }
...
int BMP_save(const BMP_Image *img, const char *filename)
{
  ...
  if (fwrite(img -> data, sizeof(char), img -> data_size, fptr) !=
      (img -> data_size))
    {
      // fwrite fails
      fclose (fptr);
