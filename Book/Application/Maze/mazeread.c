...
// check whether ptr is NULL, if so, print an error message and exit
...
static void Maze_findSize(FILE * fh, int * numRow, int * numCol)
{
  ...
  int col = 0;
  int maxCol = 0;
  // find the maximum number of columns. This allows the program to
  // handle a maze that is not rectangular.
  do
    {
...
void Maze_print(Maze * mzptr)
{
  ...
  for (row = 0; row < (mzptr -> numRow); row ++)
	...
	{
	  if (((mzptr -> curRow) == row) && ((mzptr -> curCol) == col))
	    {
