...
{
  int row = 0;
  // column should start at one because column = 0 stores the character
	     ...
	     int * * codebook, int * mapping)
{
  ...
  FILE * outfptr = fopen(outfile, "a"); // append, after the header
  if (outfptr == NULL)
    {
      fclose (outfptr);
      return 0;
...
int encode(char * infile, char * outfile)
{
  ...
  numCol ++;
  // create a two-dimensional array and initialize the codes to -1
  ...
  // a mapping table from ASCII to the indexes of the code book
...
static TreeNode * readHeader(FILE * infptr)
{
  ...
  ListNode * head = NULL;
  // decreasing to ensure the list is a stack (first in, last out)
  while (done == 0)
    {
  ...
  // unnecessary to read the remaining unused bits in the last byte
  TreeNode * root = head -> tnptr;
  // the linked list is no longer needed
  free (head);
  return root;
